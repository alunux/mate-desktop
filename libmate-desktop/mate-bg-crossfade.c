...
#else
		gdk_window_begin_paint_region (fade->priv->window, region);
		cr = gdk_cairo_create (fade->priv->window);
#endif
...
#else
		cairo_destroy (cr);
		gdk_window_end_paint (fade->priv->window);
#endif
 ...
 * mate_bg_crossfade_start() and mate_bg_crossfade_end() need to
 * be called. If animations are disabled, the crossfade is skipped,
 * and the widget background is set immediately to the end surface.
 **/
void
